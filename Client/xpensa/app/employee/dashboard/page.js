...
import { Upload, Plus, Clock, CheckCircle, XCircle, DollarSign, Calendar, FileText, Search } from 'lucide-react';
...
export default function EmployeeExpenseDashboard() {
  ...
  const submitForApproval = (id) => {
    setExpenses(expenses.map(exp => 
      exp.id === id ? { ...exp, status: 'Waiting approval', submittedDate: new Date().toISOString().split('T')[0] } : exp
    ));
  };
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      case 'Waiting approval': return 'bg-yellow-100 text-yellow-700';
  ...
  const getStatusIcon = (status) => {
    switch(status) {
      ...
      case 'Waiting approval': return <Clock className="w-4 h-4" />;
          ...
          <div className="divide-y divide-slate-200">
            {filteredExpenses.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-slate-500 text-sm mt-1">Create a new expense to get started</p>
              </div>
            ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(expense.status)}`}>
